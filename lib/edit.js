...
(function (factory) {
    ...
    */
    ...
    function wrapSection (before, lines, after) {
        return before.concat (lines.map (line => ('    ' + line)), after);
    }
    ...
    function toSparqlUpdate (datasetGraph, editsGraph, changeSet) {
        ...
        // const added = find.tools.dereify (c.allObjects (id, cs.addition));
        // const removed = find.tools.dereify (c.allObjects (id, cs.removal));

        const addedLines = added.map (tripleToSparql);
        const removedLines = removed.map (tripleToSparql);

        const editLines = c.all (null, null, null).map (tripleToSparql);
